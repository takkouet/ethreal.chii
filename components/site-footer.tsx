...
const socials = [
  ...
  {
    href: "https://www.instagram.com/chiikawa_market/",
    label: "Instagram",
  ...
  {
    href: "https://www.youtube.com/@chiikawa",
    label: "YouTube",
    ...
    <footer className="mt-auto border-t border-border bg-stripe">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-3 px-4 py-5 sm:flex-row sm:justify-between">
        ...
        <nav className="flex items-center gap-2" aria-label="Social media">
          {socials.map(({ href, label, path }) => (
            <a
              ...
              aria-label={label}
              className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-white text-ink shadow-soft hover:bg-pink-pale transition-colors duration-200 cursor-pointer"
            >
              <svg
                viewBox="0 0 24 24"
                className="h-4 w-4"
        ...
        <p className="text-xs text-muted">
          © {new Date().getFullYear()} ethereal.chii
        </p>
      </div>
    </footer>
  );
